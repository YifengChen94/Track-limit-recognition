...
from builder.frontend_builder import build_frontend
import numpy as np
import os, sys
...
def ConvUpscaleBlock(inputs, n_filters, kernel_size=[3, 3], scale=2):
    ...
    """
    net = tf.nn.relu(slim.batch_norm(inputs, fused=True,is_training=False))
...
def ConvBlock(inputs, n_filters, kernel_size=[3, 3], strides=1):
    ...
    net = tf.nn.relu(slim.batch_norm(net, fused=True,is_training=False))
    return net
...
def AttentionRefinementModule(inputs, n_filters):
    ...
    net = slim.batch_norm(net, fused=True,is_training=False)
    net = tf.sigmoid(net)
...
def build_bisenet(inputs, num_classes, preset_model='BiSeNet', frontend="ResNet101", weight_decay=1e-5, is_training=True, pretrained_dir="checkpoints"):
    """
    ...
    ### Context path
    logits, end_points, frontend_scope, init_fn  = build_frontend(inputs, frontend, pretrained_dir=pretrained_dir, is_training=is_training)
